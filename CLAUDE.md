...
- **Social media workflow**: Social Media Auto Post via Blotato (`8cYFbdjbo8s6luCA`, 16 nodes)
...
4. **Social media posting (Instagram + Twitter/X)**: Separate workflow picks random SFW media (`source_folder: "socialmedia"`), publishes via Blotato API to Instagram (@juwellia4) and Twitter/X (@juwellia4). Videos posted as reels on IG. Runs 1-2 times/day between 10-20h.
...
- `fanvue-social-media-post.json` - Social media workflow via Blotato (Instagram + Twitter/X)
...
- Dropbox, Fanvue OAuth2, OpenAI, Gmail, Google Sheets OAuth2, Blotato API
...
# Trigger a test social media post (Instagram + Twitter/X via Blotato)
